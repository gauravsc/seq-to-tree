...
depth_weights = np.log2(np.arange(2, tgt_max_seq_len+2))
depth_weights = torch.tensor(depth_weights/np.sum(depth_weights), dtype=torch.float).to(out_device)
	...
	for ep in range(n_epochs):
		...
		for it, file in enumerate(files):
			...
			while i < file_size:
				...
				for rs in range(int((1.0*batch_size*15)/max_batch_size)):
		...
		if save_model:
			torch.save(transformer.state_dict(), '../saved_models/with_weights_model.pt')
...
def main():
	...
	if load_model and os.path.isfile('../saved_models/with_weights_model.pt'):
		transformer.load_state_dict(torch.load('../saved_models/with_weights_model.pt'))
